    ...
    <el-table border stripe v-loading="listLoading" :data="users" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="{row}">
          <tipsButton type="info" size="mini" icon="el-icon-user-solid" title="分配角色" @click="showAssignRole(row)" />
          <tipsButton type="primary" size="mini" icon="el-icon-edit" title="修改用户" @click="showUpdateUser(row)" />
          <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
            <tipsButton style="margin-left:10px" slot="reference" type="danger" size="mini" icon="el-icon-delete" title="删除用户" />
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    */
    async getRoles() {
      const result = await this.$rApi.user.getRoles(this.user.id)
    ...
    */
    async assignRole() {
      ...
      this.loading = true
      const result = await this.$rApi.user.assignRoles(userId, roleIds)
      this.loading = false
    ...
    */
    revomveUsers() {
      this.$confirm('确定删除吗?').then(async () => {
        await this.$rApi.user.removeUsers(this.selectedIds)
    ...
    */
    async removeUser(id) {
      await this.$rApi.user.removeById(id)
      this.$message.success('删除成功')
    ...
    */
    async getUsers(page = 1) {
      ...
      const result = await this.$rApi.user.getPageList(page, limit, searchObj)
    ...
    addOrUpdate() {
      this.$refs.userForm.validate(valid => {
        if (valid) {
          const { user } = this
          this.loading = true
          this.$rApi.user[user.id ? 'update' : 'add'](user).then((result) => {
            this.loading = false
