  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{row}">
          <tipsButton size="mini" type="info" icon="el-icon-info" title="分配权限"
            @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)"/>

          <tipsButton size="mini" type="primary" icon="el-icon-check" title="确定" 
            @click="updateRole(row)" v-if="row.edit"/>
          <tipsButton size="mini" type="primary" icon="el-icon-edit" title="修改角色" 
            @click="row.edit= true" v-if="!row.edit"/>

            
          <tipsButton size="mini" type="danger" icon="el-icon-delete" title="删除角色"
...
export default {
  ...
  methods: {
    ...
    */
    updateRole (role) {
      this.$rApi.role.updateById({id: role.id, roleName: role.roleName})
        .then(result => {
    ...
    */
    addRole(){
      ...
      this.$prompt('请输入新名称', '添加角色', {
        ...
        this.$rApi.role.save({roleName: value}).then(result => {
    ...
    */
    getRoles(page = 1) {
      ...
      this.$rApi.role.getPageList(page, limit, searchObj).then(
        result => {
    ...
    */
    removeRole ({id, roleName}) {
      this.$confirm(`确定删除 '${roleName}' 吗?`, '提示', {
        ...
        const result = await this.$rApi.role.removeById(id)
    ...
    */
    removeRoles () {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        const result = await this.$rApi.role.removeRoles(ids)
        this.getRoles()
        this.$message({
          type: 'success',
          message: '批量删除成功!'
